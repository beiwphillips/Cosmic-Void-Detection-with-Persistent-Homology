...
class TopologicalUnionFind(object):
    ...
    def __init__(self, elements=None):
        ...
        self._birth = []  # birth time of the component
        self._death = []  # death time of the component
        self._history = [] # history of the component
    ...
    def add(self, x, time):
        ...
        self.n_elts += 1
        self.n_comps += 1
        self._birth.append(time)
        self._death.append(float('inf'))
        self._history.append({time: [x.getCoordinates()]})
    ...
    def union(self, x, y, time):
        ...
        if xroot == yroot:
            return
        if self._birth[xroot] > self._birth[yroot]:
            self._par[xroot] = yroot
            self._siz[yroot] += self._siz[xroot]
            self._death[xroot] = time
            self._history[yroot][time] = self._history[yroot].get(time, [])+[c for l in self._history[xroot].values() for c in l]
        else:
            self._par[yroot] = xroot
            self._siz[xroot] += self._siz[yroot]
            self._death[yroot] = time
            self._history[xroot][time] = self._history[xroot].get(time, [])+[c for l in self._history[yroot].values() for c in l]
        self.n_comps -= 1
    ...
    def persistence(self):
        """Return the birth time and death time for connected components.

        Returns
        -------
        list
            A list of tuples of format (birth, death).

        """
        result = []
        for i in range(self.n_elts):
            # print(self._elts[i], self._birth[i], self._death[i])
            if self._birth[i] == self._death[i]:
                continue
            result.append((self._birth[i], self._death[i]))
        return result

    def persistence_history(self):
        """Return the birth time and death time for connected components.
        ...
        for i in range(self.n_elts):
            ...
            if self._birth[i] == self._death[i]:
                continue
            result['bar-'+str(len(result))] = {'birth': self._birth[i], 
                                          'death': self._death[i], 
